...
// Utilizador a escolher dificuldades
...
function game() {
    ...
    if (ball.x + ball.radius + ball.velocity.x > canvas.width) {
        // Se a bola tocar na "parede" direita, ponto para o user.
        resetBall();
        userScore.play();
        console.log(userScore);
        user.score += 1;
    } else if (ball.x - ball.radius < 0) {
        // Se a bola tocar na "parede" esquerda, ponto para o computador
        resetBall();
        comScore.play();
    ...
    if (collision(ball, player)) {
        // Primeiro, pegar o ponto em que a raquete atingiu a bola.
        let collidePoint = ball.y - (player.y + player.height / 2);

        // Em seguida, converter em um número que será de 1 a -1.
        collidePoint = collidePoint / (player.height / 2);

        // Em seguida, multiplicar por 45 graus para que possamos obter um ângulo perfeito entre eles.
        // 45 e -45.
        ...
        // Adicionar .5 à bola para que fique mais rápido
        ball.speed += 0.5;
...
play.addEventListener("click", () => {
    ...
    if (mode.value == "") {
        ...
        alert("Escolhe o modo jogo primeiro");
        panel.classList.add("reveal");
        return;
    }
