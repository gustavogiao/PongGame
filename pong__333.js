  ...
  drawSplashScreen(paddle) {
    ...
    if (paddle != null) {
      const victoryString = !paddle.isRightPaddle
        ? "Jogador 1 Ganhou!"
        : "Jogador 2 Ganhou";
      this.ctx.fillText(victoryString, 120, 100);
    }

    this.ctx.fillText("Espaço para jogar", 120, 300);
    this.ctx.save();
...
class Paddle {
  ...
  move(movementDirection) {
    ...
    if (
      ...
      this.yPos < gameCore.ctx.height - this.height / 0.95
    ) {
      this.yPos += 7;
    }
  }
